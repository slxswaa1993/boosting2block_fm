...
class QuadraticSolver():
    ...
    def getG_ration(W, W_old,P,Q,C):
        ...
        ########下面为正常代码######
        if isinstance(C,sp.csr.csr_matrix):
            temp=P-Q+C
        else:
            temp=P-Q
    ...
    def costfun_lambda_t(H,a_3,U,lambda_t,G): 
            ...
            '''
            #print 'costfun_lambda_t U.shape',type(U)
            # ndarray
             #  F=(H-a_3)*G*(U)*(exp)
            exp=np.exp(-lambda_t*H)
            F=np.multiply(H-a_3,G)
            F=np.multiply(F,U)
            F=np.multiply(F,exp) 
            cost_lambda_t=np.sum(F)           
...
#######################################################
    ...
    @staticmethod
    def updateU(lambda_t,U,z_t,W, W_old, X_uv, X_uf,C):
        '''
        ...
        H=QuadraticSolver.getH(z_t,X_uv,X_uf)        
        exp=np.exp(-lambda_t*H)        
        #U=np.multiply(U,exp_G)
        U=np.multiply(U,G)
        U=np.multiply(U,exp)
        
        #######按照用户组进行更新########
        #user_ids = np.array([u for u, i in ada_bprtrain_data])
        #user_ids = np.unique(user_ids)
        #start=0
        #for u in user_ids:
        #    items_num=Tr[u]['num']
        #    U[start:start+items_num]=U[start:start+items_num]/np.sum(U[start:start+items_num])
        #    start+=items_num
        ###############
